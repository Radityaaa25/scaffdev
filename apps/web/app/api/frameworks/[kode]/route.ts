...
export async function GET(_request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const { data } = await supabase
      ...
      .single();
    if (!data) {
      return NextResponse.json({ error: "Framework tidak ditemukan." }, { status: 404 });
    }
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  ...
  if (
    ...
    return NextResponse.json({ error: "Kode framework tidak dapat diubah." }, { status: 400 });
  }

  const validated = validateReferensiInput(body, { partial: true, entity: "Framework" });
  if (!validated.ok) {
  ...
  if (error) {
    return NextResponse.json({ error: "Gagal memperbarui framework." }, { status: 500 });
  }

  await logActivity(supabase, email, "framework.update", "referensi", data.kode, data.nama_tampilan);
  return NextResponse.json({ framework: data });
}
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  ...
  const { count } = await supabase
    ...
    .eq("framework", kode.toLowerCase());
  if ((count ?? 0) > 0) {
    return NextResponse.json(
      { error: `Tidak bisa dihapus: masih dipakai ${count} template. Pindahkan dulu template terkait ke framework lain.` },
      { status: 409 }
    );
  }
  ...
  const { error } = await supabase
    ...
    .eq("kode", kode.toLowerCase());
  if (error) {
    return NextResponse.json({ error: "Gagal menghapus framework." }, { status: 500 });
  }

  await logActivity(supabase, email, "framework.delete", "referensi", existing.kode, existing.nama_tampilan);
