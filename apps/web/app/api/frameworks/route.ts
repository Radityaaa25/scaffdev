...
export async function GET() {
  // Daftar framework bersifat publik (filter katalog + dropdown CLI jujur).
  try {
    ...
    const { data, error } = await supabase
      ...
      .order("nama_tampilan");
    if (error) {
      return NextResponse.json({ error: "Gagal mengambil data framework." }, { status: 500 });
    }
...
export async function POST(request: NextRequest) {
  ...
  const validated = validateReferensiInput(body, { partial: false, entity: "Framework" });
  if (!validated.ok) {
  ...
  if (error) {
    if (error.code === "23505") {
      return NextResponse.json(
        { error: `Kode "${validated.data.kode}" sudah dipakai framework lain.` },
        { status: 409 }
      );
    }
    return NextResponse.json({ error: "Gagal menyimpan framework." }, { status: 500 });
  }

  await logActivity(supabase, email, "framework.create", "referensi", data.kode, data.nama_tampilan);
